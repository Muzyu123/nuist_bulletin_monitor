...
## 功能亮点
> - 适用新网站。代码构建于2026年9月
> - 不需要启动，基于Linux crontab（定时任务）保证后台存活，无需每次开机后手动启动，不受用户登出、关闭终端甚至服务器重启影响
> - 一键化配置，无需复杂部署
> - 极致轻量化，几乎不影响服务器性能
...
## 写在前面
1.本意是为了帮舍友关注普通话考试报名的相关通知，顺便留意一下中秋和国庆的放假通知
结果没想到这个程序第一次真正测试成功就是命中了学校通知中秋国庆连放13天的通知（南信伟大无需多言）
2.运行测试环境:
AlmaLinux 9.5 (Teal Serval) ：本项目依赖于 Linux 定时任务实现保活
Python >= 3.8：仅使用 Python 标准库，无需引入第三方库
3.本项目部分使用 Claude Code + 哦鲸鲸V4.1Flash 构建
